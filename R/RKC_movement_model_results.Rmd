...
# movement directions
compute_bearing <- function(start_easting, start_northing, end_easting, end_northing) {
  dx <- end_easting - start_easting
  dy <- end_northing - start_northing
  ...
  # adjust angle to compass bearing (0 to 360 degrees)
  bearing <- (angle_deg + 360) %% 360
  
  return(bearing)
}

# bearing for crab movement (all tags)
oos_obs_movement$bearing <- compute_bearing(start_easting = oos_obs_movement$deploy_lon,
                              start_northing = oos_obs_movement$deploy_lat,
                              end_easting = oos_obs_movement$lon0,
                              end_northing = oos_obs_movement$lat0)
...
summ_stat_circ(oos_obs_movement$bearing,
               stat = "mean")
summ_stat_circ(oos_obs_movement$bearing,
               stat = "sd")
...
min_depth <- env_cov_full_df %>%
  pull(depth) %>% 
  min()

max_depth <- env_cov_full_df %>% 
  pull(depth) %>%
  max()

min_tc <- env_cov_full_df %>% 
  pull(tidal_curr) %>% 
  min()

max_tc <- env_cov_full_df %>%
  pull(tidal_curr) %>% 
  max()

mean_temp <- env_cov_full_df %>%
  filter(year %in% c(2021:2023)) %>% 
  pull(temp) %>% 
  mean()

# prediction grid where temperature is held at the mean (for 2D pref. figure)
pref_surf <- 
  tibble(tidal_curr = seq(min_tc, max_tc, length.out = 50)) %>% 
  expand_grid(depth = seq(min_depth, max_depth, length.out = 50)) %>% 
  mutate(temp = mean_temp) 

# Extract model components
Xpred <- predict(f_mod1, newdata = pref_surf, type = "lpmatrix")
...
pref_surf$preference <- as.numeric(scale(Xpred %*% ests, center = T, scale = F))

# extract range of tidal currents and depths where preference is high
pref_surf %>% 
  filter(preference > quantile(pref_surf$preference)[4]) %>% 
  pull(tidal_curr) %>% 
  range()
...
# constrain the prediction grid for visualizing
pref_surf_constrained <- pref_surf %>%
    rowwise() %>%
    mutate(
        ok = any(
            (abs(tidal_curr - env_cov_full_df_2021$tidal_curr) <= 
               env_cov_full_df_2021$tidal_curr * 0.1) &
                (abs(depth      - env_cov_full_df_2021$depth     ) <= 
                   abs(env_cov_full_df_2021$depth) * 0.1)
        ),
        preference = if_else(ok, preference, NA_real_)
    ) %>%
    ungroup() %>%
    dplyr::select(-ok) %>% 
  filter(depth >  22.5)
...
# marginal smooth effect of bottom temperature on preference
temp_effect <- 
  effectDT(model = pref_mod,
           df = env_cov_full_df %>% filter(year %in% 2021:2023),
            terms = "temp") 
...
temp_smooth_plt <-
  ...
  geom_rug(data = combined_env,
           ...
           sides = "b", alpha = 0.6) +
  geom_ribbon(aes(ymax = pref_upr, ymin = pref_lwr, x = x), alpha = 0.2) +
  geom_line(aes(y = pref, x = x)) + 
...
# 2D marginal smooth of depth-tidal current interaction
fill_lims <- range(pref_surf_constrained$preference)
depth_tc_effect_plt <- 
  ggplot(pref_surf_constrained,aes(y = depth, 
                     x = tidal_curr)) +
  geom_raster(aes(fill = preference), show.legend = T) + 
  geom_contour(aes(z = preference), color = "black") + 
...
pref_surf_constrained %>% 
  filter(tidal_curr > median(env_cov_full_df$tidal_curr, na.rm = T),
         preference > quantile(pref_surf_constrained$preference, na.rm = T,
                               0.90)) %>% 
  pull(depth) %>% 
  range()
...
sim_out_diff_v2.1 <- sim_out_diff_v2 %>% 
  filter(!is.na(bhat_proj))
...
sim_out_diff_v2.1$sim_dists_proj <- sim_dists_proj
sim_out_diff_v2.1$sim_dists_diff <- sim_dists_diff

## overlap
bhat_p <- sim_out_diff_v2.1 %>%
  mutate(D = bhat_proj - bhat_diff) %>%
  group_by(year) %>%
  summarise(
    p_lower = mean(D <= 0), # one-sided P value
  )%>%
  dplyr::select(year, p = p_lower)

## distances to fishing CPUE centroid using diffusion-taxis or diffusion only models
dist_p <- sim_out_diff_v2.1 %>%
  mutate(D = sim_dists_proj - sim_dists_diff) %>%
  group_by(year) %>%
  summarise(
    p_upper = mean(D >= 0), # one-sided P value
  ) %>%
  dplyr::select(year, p = p_upper) 

# how frequently and to what extent do diffusion-taxis predicted tag movements
# get closer to actual tag pop-up locations than predicted tag pop-up locations
# from the diffusion taxis model alone?
dist_sim_out_v2 %>%
  dplyr::select(td_distance, diff_distance, year, tag) %>%
  mutate(distance_difference = td_distance - diff_distance) %>%
  group_by(year) %>%
  dplyr::summarise(perc_closer = sum(td_distance < diff_distance)/n(),
                   avg_dist = mean(distance_difference))

# average distance between predicted and observed for diffusion-taxis and diffusion only
dist_sim_out_v2 %>%
  dplyr::select(td_distance, diff_distance, year, tag) %>% 
  group_by(year) %>% 
  dplyr::summarise(mean_td_distance = mean(td_distance),
                   mean_diff_distance = mean(diff_distance))

# average distance among all years and tags
dist_sim_out_v2 %>%
  dplyr::select(td_distance, diff_distance, year, tag) %>% 
  dplyr::summarise(mean_td_distance = mean(td_distance),
                   mean_diff_distance = mean(diff_distance))
