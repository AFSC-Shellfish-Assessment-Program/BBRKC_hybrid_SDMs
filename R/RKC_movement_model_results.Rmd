...
max(distance_traveled$dist)
min(distance_traveled$dist)
...
# depth and tidal current comparison
depth_comp <- 
  combined_env %>%
...
depth_comp %>% 
  group_by(loc) %>% 
  dplyr::summarise(m_tidal_curr = mean(tidal_curr),
                   sd_tidal_curr = sd(tidal_curr))
```
