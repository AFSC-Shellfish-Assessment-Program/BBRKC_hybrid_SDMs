...
# sf object limited from 2021-2024
...
mod_tab <-
  ...
  filter(did_converge %in% c("relative convergence (4)",
...
round(mean(distance_traveled$dist))
round(sd(distance_traveled$dist))

# max/min distance traveled
round(max(distance_traveled$dist))
round(min(distance_traveled$dist))

# calculate bearing from start and end locations
bear_out <- NULL
for (i in unique(sf_s0_locs$tag)){
  start <- sf_s0_locs %>% filter(tag == i) %>% 
    st_transform(., crs = 4326)
  end <- sf_s1_locs %>% filter(tag == i)  %>% 
    st_transform(., crs = 4326)
  bear_int <- as.numeric(st_geod_azimuth(bind_rows(start, end))  * (180/pi))
  assign("bear_out", rbind(bear_out, 
                           tibble(bearing = ((bear_int %% 360) + 360) %% 360, #0-360
                                            tag = i)
                           )
         )
}
...
# direction of movement across all tags
round(summ_stat_circ(oos_obs_movement2$bearing,
               stat = "mean"))
round(summ_stat_circ(oos_obs_movement2$bearing,
               stat = "sd"))
...
# count tags released in closure areas that popped-up in closure areas
st_intersection(tsf,
                sf_s1_locs %>% 
...
oos_obs_movement2 %>% 
  filter(tag %in% init$tag) %>% 
  dplyr::summarise(m_bearing = round(summ_stat_circ(bearing, stat = "mean")),
                   sd_bearing = round(summ_stat_circ(bearing, stat = "sd")))

# crab distance traveled when released in closure areas
distance_traveled %>% 
  filter(tag %in% init$tag) %>% 
  dplyr::summarise(m_dist = round(mean(dist)),
                   sd_dist = round(sd(dist)))
...
round((nrow(depth_change %>% filter(depth_change < 0))/nrow(depth_change))*100)

shallower_tags <- depth_change %>% filter(depth_change > 0) %>% pull(tag) 

# depth and tidal current comparison across -164 W
depth_comp <- 
  combined_env %>%
  mutate(loc = factor(ifelse(tag %in% west_tags, "west", "east")))

t.test(depth ~ loc, data = depth_comp)

depth_comp %>% 
  group_by(loc) %>% 
  dplyr::summarise(m_depth = round(mean(depth)),
                   sd_depth = round(sd(depth)))

t.test(tidal_curr ~ loc, data = depth_comp)

depth_comp %>% 
  group_by(loc) %>% 
  dplyr::summarise(m_tidal_curr = round(mean(tidal_curr)),
                   sd_tidal_curr = round(sd(tidal_curr)))
```
...
df_grid <- env_cov_full %>% filter(year %in% 2021:2024, month == 10)
...
# exclude predictions too far from observations
tf <- exclude.too.far(pred_int$tidal_curr,
                      pred_int$depth,
                      combined_env$tidal_curr,
                      combined_env$depth,
                    0.15)

pred_int2 <- pred_int[!tf,]
# The x axis for the temperature preference plot ranges between the minimum and maximum
# October temperatures for 2021-2024. We don't extend the preference function lower
# than the lower bound of October temperatures, because the bottom temperatures at tag deployment
# in June don't occur in October, so crab cannot choose to go towards them. The situation is different
# in the 2D plot of preference, where covariates are static. In this case, crab can choose to stay 
# in the temperature/current velocity habitat that the occupy in June. Therefore, we extend the 
# preference surface to encompass preferences at both tag deployment and pop-up locations, assuming that
# the preference space in between deployment and pop-up locations is fair game. We
# also add a buffer to the points to show predicted preference at depth/current velocity combinations
# near deployment and pop-up, presumably habitats that crab could have moved into but didn't.
...
depth_tc_effect_plt <- 
  ggplot(data = pred_int2,
    ...
    geom_contour(aes(z = estimate), color = "black", bins = 15) +
...
# predict habitat preference from fitted model
pref_surf <- 
  env_cov_full_df %>% 
      filter(year %in% 2021:2024,
             month == 10) %>% 
...
spat_pref <- env_cov_full %>% 
  ...
  st_as_sfc() %>%
  st_as_sf() %>%
  expand_grid(year = 2021:2024) %>%
  arrange(year) %>%
  mutate(preference   = pref_surf$preference) %>% 
  group_by(year) %>% 
  mutate(
    ...
    pref_dec2 = factor( ntile(preference, n = 10) * 10 )
  ) %>% 
  st_as_sf() %>% 
  st_transform(., crs = 4326)
...
pref_map <-
  ggplot() +
    geom_sf(data = spat_pref ,
            aes(fill = pref_dec), color = "transparent") +
  ...
  ggnewscale::new_scale_color() +
  geom_sf(data = nbbtca, color = "blue",
          linewidth = 0.5,
          linetype= 2,
          show.legend = F,
          fill = "transparent") +
...
```{r}
# find the fraction of CPUEs that occurred within the boundary of the prediction 
# grid by year
pg_outer <- 
  pred_grid %>%
...
inside_catches <- NULL
catch_yrs <- unique(lb$year)
for (i in seq_along(catch_yrs)){
  lb_int <- lb %>% filter(year == catch_yrs[i])
  lb_inside <- nrow(st_intersection(lb_int, pg_outer))
  prop_inside <- round(lb_inside/nrow(lb_int), 4) * 100
  assign("inside_catches", rbind(inside_catches, 
                                 tibble(year = catch_yrs[i],
                                        prop_inside,
                                        n_inside = lb_inside,
                                        n_outside = nrow(lb_int) - lb_inside)))
}
inside_catches
```
  ...
  message(y)
    ...
    if (y != 2020){
      ...
      ann_proj_int <- project_distribution(
                                   year = y,
                                   month = 10,
                                   temp_data = env_cov_full %>% filter(month == 10),
...
```{r}
# visualize projections and harvests-----
facets_subset <- c(2018:2019, 2023:2024)
pal <- rev(pals::ocean.deep(100))
...
# all years for supplement
facet_catch <- c(2005:2020, 2023:2024)
...
```{r}
# see R/sims/reconcile_sims.R and overlap_sims scripts
...
bhat_eta <- NULL
for (i in c(2005:2019, 2023:2024)) {

  # print(dist_comps %>% filter(year == i) %>% pull(bhat_diff) %>% unique() %>% length())
...
ovlp_ts_v2.1 %>% 
  dplyr::select(year, mean_cog_lon, cog_lon_catch) %>% 
  mutate(diff = mean_cog_lon - cog_lon_catch) %>% 
  arrange(year)

ovlp_ts_v2.1 %>% 
  dplyr::select(year, mean_cog_lat, cog_lat_catch) %>% 
  dplyr::summarise(mean_cog_lat = mean(mean_cog_lat, na.rm = T),
                   cog_lat_catch = mean(cog_lat_catch, na.rm = T)) %>% 
  mutate(diff = mean_cog_lat - cog_lat_catch)
  ...
  geom_sf(data = nbbtca, color = "red",
    ...
    geom_errorbar(aes(xmin = lon_upr95_cog,
      ...
      geom_errorbar(aes(ymin = lat_upr95_cog,
                ...
                color = "gray",
                width = 0) +
    geom_point(aes(y = lat_mean_cog, x = lon_mean_cog, color = t_m, group = year), size = 4) +
