...
for (i in all_years){
  ...
  if (i %in% survey_years){
    ...
    for (j in 1:nsim){
      ...
      assign("cog_out_diff.3", rbind(cog_int, cog_out_diff.3))
...
for (yr in all_years){
  ...
  assign("sim_out_diff_v2.3", rbind(int2, sim_out_diff_v2.3))
}
...
save(sim_out_diff_v2.3,
     dist_comps.3,
     ovlp_ts_v2.3,
     cog_out_diff.3,
     file = here::here("data/ovlp_sims3.rdata"))
