...
for (i in all_years){
  ...
  if (i %in% survey_years){
    ...
    for (j in 1:nsim){
      ...
      assign("cog_out_diff.4", rbind(cog_int, cog_out_diff.4))
...
for (yr in all_years){
  ...
  assign("sim_out_diff_v2.4", rbind(int2, sim_out_diff_v2.4))
}
...
save(sim_out_diff_v2.4,
     dist_comps.4,
     ovlp_ts_v2.4,
     cog_out_diff.4,
     file = here::here("data/ovlp_sims4.rdata"))
